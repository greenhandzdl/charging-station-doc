...
| Mock充电机 | **Swing 桌面客户端** | 模拟充电交互（插枪/拔枪/刷卡启动），充电进度显示 |

### Mock充电机客户端说明

Mock充电机客户端是前后端不分离的 Swing 桌面客户端，用于模拟物理充电机与用户的交互。其主要职责包括：

- **模拟物理操作**：提供插枪按钮、拔枪按钮、刷卡启动等 Swing UI 组件，模拟用户在真实充电机上的操作
- **充电进度展示**：包含 JProgressBar 进度条、电量标签、费用标签，实时显示充电数据
- **模拟电量生成**：内置 ChargeSimulator 模拟引擎，每 tick 增加 0.1kWh 电量，记录充电时长
- **调用后端 API**：通过 HTTP 调用 `/api/v1/charges/start` 和 `/api/v1/charges/stop` 执行充电业务
