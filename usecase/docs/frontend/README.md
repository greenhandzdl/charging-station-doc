...
| Mock充电机（普通模式） | **Swing 桌面客户端** | 模拟充电交互（插枪/拔枪/生成二维码），使用 mock_user/mock123 登录，测试场景模拟 |
| Mock充电机（高级模式） | **Swing 桌面客户端** | 使用 ADVANCED_API_KEY 密钥认证，可见所有充电桩及中间件交互，UI 显示[高级模式]，仅测试环境开放 |

### Mock充电机客户端说明

Mock充电机客户端是前后端不分离的 Swing 桌面客户端，用于模拟物理充电机的面板显示与交互。支持**两种运行模式**：
