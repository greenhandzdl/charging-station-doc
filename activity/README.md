...
| 泳道 | 职责 |
|------|------|
| Mock充电机客户端 | 启动 Swing 界面、选择充电桩、模拟插枪/拔枪、显示充电进度条、调用后端充电 API、展示充电结果 |
| 用户 | 确认启动/结束充电操作 |
| 管理员 | 强制结束充电、填写终止原因 |
...
1. Mock充电机客户端启动 Swing 界面，显示"请选择充电桩"面板
2. 用户通过 Flutter 选择充电站与充电桩，点击"启动充电"
3. Mock充电机客户端显示"模拟插枪"界面，用户点击"模拟插枪"按钮
4. Mock充电机客户端调用后端启动充电 API
5. 系统校验：用户身份（JWT）、余额（>= 10元）、桩状态（空闲）
6. 系统锁定充电桩（status → charging），创建充电记录（status: processing）
7. Mock充电机客户端启动电量模拟（0.1kWh/秒），实时更新充电面板
8. 系统进入三种结束分支之一：
   - **正常结束**：用户通过 Mock充电机模拟拔枪 → 停止电量模拟 → 后端计算费用 → 校验余额 → 事务扣费（成功或标记欠费）→ 释放桩 → Mock充电机显示结果
   - **强制结束**：管理员填写原因 → 权限校验 → 事务结束记录 → 释放桩 → 审计日志记录原因 → Mock充电机显示终止通知
