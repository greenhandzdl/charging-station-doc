#!/bin/bash
#
# dev-web.sh — 启动 Flutter Web Server (开发模式)
#
# 用法:
#   ./dev-web.sh
#
...
#   2. 启动 flutter run -d web-server (监听 0.0.0.0:8081，使用 --no-dds)
#   3. 记录 PID 到 /tmp/flutter-web-dev.pid
#   4. 执行健康检查 (60 秒超时)

set -euo pipefail

APP_DIR="$(cd "$(dirname "$0")/../code/charging-station-client" && pwd)"
PID_FILE="/tmp/flutter-web-dev.pid"
HOST="0.0.0.0"
PORT="8081"
TIMEOUT=60
...
echo "[INFO] 启动 Flutter Web Server (开发模式): $HOST:$PORT"
echo "[INFO] 工作目录: $APP_DIR"
echo "[INFO] 参数: --no-dds (禁用 Dart Development Service, 便于调试)"

# 启动 Flutter web server (开发模式) 在后台
flutter run -d web-server \
    --web-hostname="$HOST" \
    --web-port="$PORT" \
    --no-dds \
    > >(tee /tmp/flutter-web-dev.log) 2>&1 &
FLUTTER_PID=$!
...
while [ $ELAPSED -lt $TIMEOUT ]; do
    ...
    if ! kill -0 "$FLUTTER_PID" 2>/dev/null; then
        ...
        echo "[ERROR] 日志:"
        tail -20 /tmp/flutter-web-dev.log
        rm -f "$PID_FILE"
        exit 1
    fi
...
echo "[ERROR] 日志:"
tail -30 /tmp/flutter-web-dev.log
rm -f "$PID_FILE"
exit 1